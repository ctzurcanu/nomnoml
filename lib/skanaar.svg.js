...
skanaar.Svg = function(svg, callbacks){
	...
	var ctx = {
		...
		fillText: function(text, x, y){
			...
			var out = svg.text(text).move(x+ctx.i.x,y+ctx.i.y).attr({ "text-anchor":ctx.textAlign, "font-family": ctx.config.font,"font-size":ctx.config.fontSize})
	...
	var tracePath =  function(path, offset, s, close){ //[{x:x, y:yDivider}, {x:x+node.width, y:yDivider}]
			...
			svg.path(path1).attr({fill:ctx.fillStyle,stroke:ctx.config.stroke,"stroke-width":ctx.lineWidth})
		}
	...
	return {
		...
		ellipse: function (center, rx, ry, start, stop){
			...
			svg.ellipse(rx,ry).move(center).attr({fill:ctx.fillStyle,stroke:ctx.config.stroke,"stroke-width":ctx.lineWidth})
		/*
			ctx.beginPath()
			ctx.save()
		...
		arc: function (x, y, radius, startAngle, endAngle){
		    ...
		    console.info(x, y, radius, startAngle, endAngle)

			//var path ="M "+x+" "+y+"m "-r+", 0 a "+r+","+r+" 0 1,1 "+(r * 2)+",0 a "+r+","+r+" 0 1,1 "-(r * 2)+",0";
			svg.path(d).attr({fill:ctx.fillStyle,stroke:ctx.config.stroke,"stroke-width":ctx.lineWidth})
			//return chainable
		},

		roundRect: function (x, y, w, h, r){
			svg.rect(w,h).attr({rx:r,ry:r}).move(x,y).attr({fill:ctx.fillStyle,stroke:ctx.config.stroke,"stroke-width":ctx.lineWidth})
			/*
			ctx.beginPath()
			ctx.moveTo(x+r, y)
